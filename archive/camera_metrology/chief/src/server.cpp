...
// It is certainly more conventional to use a C++ version of ZMQ, but this
// works...
#include <zmq.h>
...
#define MESSAGE_LATER                                                          \
    2 /* Signal from a cmd_ function that a message will be returned later,    \
...
#include "helpstring.include"
    if (argc == 1) {
        ...
        this->Message("Help for individual commands not implemented yet. "
                      "Awaiting TOML library choice.");
        return WARNING;
    }
}

// Add additional commands here with the same Cmd_ syntax as above.
int Server::Cmd_get_config(int argc, char **argv) {
    // Get the current chief configuration.
    //----------------------------------------
    if (argc == 1) {
        auto sstream = std::stringstream{};
        sstream << toml::json_formatter{serverfunc::get_config()};
        this->Message(sstream.str().c_str());
    } else {
        this->Message("Get camera config");
        return WARNING;
    }
    return NOERROR;
}

int Server::Cmd_get_led(int argc, char **argv) {
    // Get led positions captured by chief.camera
    //----------------------------------------
    if (argc == 1) {
        this->Message(serverfunc::get_led_location_string().c_str());
    } else {
        this->Message("Get led positions");
        return WARNING;
    }
    return NOERROR;
}

int Server::Cmd_get_orientation(int argc, char **argv) {
    // get from star tracker and plate solver
    // need coordinate transformation between non standard
    // coordinates in real server
    // just return a quaternion for mockup
    //----------------------------------------
    if (argc == 1) {
        this->Message(serverfunc::get_self_orientation().c_str());
    } else {
        this->Message("Get orientation quaternion of the robot");
        return WARNING;
    }
    return NOERROR;
}
...
int Server::GetNextCommand() {
    ...
    if (select(1, &fds, NULL, NULL, &tv) > 0) {
        ...
        /* Receive from ZMQ */
        nr = zmq_recv(this->responder, command, COMMAND_BUFFERSIZE - 1,
                      ZMQ_DONTWAIT);
        if (nr > 0)
