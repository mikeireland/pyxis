/*
 * This server borrows heavily from CHARA and SUSI interferometer code, which
 was
 * written in C. Therefore, even though this is a prototype, it doesn't conform
 to
...
int main(int argc, char **argv) {
    serverfunc::load_config("config.toml");
    // Import and initialise the server. This is ideally the only line to be
    // changed
    // when upgrading to a real server!!! All initialisations should occur
    // within the Server constructor.
    ...
    while (retval != FATAL) {
        ...
        if (main_server.CommandAvailable()) {
            ...
            usleep(1000);
        /* Now do the background tasks. NB, these could be an array of tasks
        only done sometimes...  details up to the server implementation. */
        main_server.RunBackgroundTasks();
    }
    // Cleanup functions should automatically be invoked here when the
    // main_server goes out of scope.
    return 0;
}
