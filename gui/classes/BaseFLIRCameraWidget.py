...
class BaseFLIRCameraWidget(QWidget):
    def __init__(self, config, IP='127.0.0.1', parent=None):

        super(BaseFLIRCameraWidget,self).__init__(parent)
    ...
    def ask_for_status(self):
        ...
        #connected
        response = self.socket.send_command("FST.status")
    ...
    def get_params(self):
        """Ask for status for the server that applies to the current tab (as we can
        only see one server at a time)"""
        command = "FST.getparams"

        if (self.socket.connected):

            response = self.socket.send_command("FST.getparams")
    ...
    def connect_camera(self):
        ...
        if self.Connect_button.isChecked():
            ...
            self.send_to_server("FST.connect")
            time.sleep(2)
            self.get_params()
            ...
            self.send_to_server("FST.disconnect")
    ...
    def reconfigure_camera(self):
        if self.Connect_button.isChecked():
            if self.run_button.isChecked():
                ...
                self.send_to_server("FST.reconfigure_all [%s]"%response_str)
    ...
    def run_camera(self):
        ...
        if self.Connect_button.isChecked():
            if self.run_button.isChecked():
                ...
                self.send_to_server("FST.start [%s,%s]"%(num_frames,self.coadd_flag))
            else:
                self.run_button.setText("Start Camera")
                print("Stopping Camera")
                self.send_to_server("FST.stop")
    ...
    def get_new_frame(self):
        ...
        response = self.socket.send_command("FST.getlatestimage [%s,%s]"%(self.compression_param,self.feed_window.binning_flag))
