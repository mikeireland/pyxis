	...
	*/
    ...
    int receive_AlignmentError(double z_offset, double y_offset) {
        ...
        if (y_offset < 1e-6 && y_offset > -1e-6) {
            ...
            y_vel = y_offset/10.0; // Divide by 10 to reduce the speed
            if (y_vel > 3.0) y_vel = 3.0;
            if (y_vel < -3.0) y_vel = -3.0;//cap the velocity to 10mm/s
        }
...
{ 
    ...
    m.instance<RobotControlServer>("RC")
        ...
        .def("track", &RobotControlServer::track_robot, "Level the robot, and track the star, depending on star tracker state.")
