...
def refraction_angle(i,wave):
    glass_n = nglass(wave,"bk7")
    prism_angle = 45 
    r_1 = snell(i,1,glass_n)
    i_2 = prism_angle - r_1
    return snell(i_2,glass_n,1)

def refraction_wave(i,wave):
    glass_n = nglass(wave,"bk7")
    prism_angle = 45 
    r_1 = snell(i,1,glass_n)
    i_2 = prism_angle - r_1
...
def snell2(i,r,n1):
    return n1*np.sin(np.radians(i))/np.sin(np.radians(r))

def snell(i,n1,n2):
    return np.degrees(np.arcsin(n1/n2*np.sin(np.radians(i))))

def pixel_location(wave):
    central_wavelength = 0.66 #Microns
    central_angle = 35.4
    focus_lens = 15*1e-3 #microns
    pix_size = 6.5
    a = np.radians(refraction_angle(central_angle,central_wavelength) - refraction_angle(central_angle,wave))
    pixel = a*focus_lens/(pix_size*1e-6)
    return pixel

def wave_location(px,offset=0):
    central_wavelength = 0.66 #Microns
    central_angle = 35.4
    focus_lens = 15*1e-3 #microns
    pix_size = 6.5
    a = 0.6182901
    angle_diff = (px-offset)*(pix_size*1e-6)/focus_lens
    new_a = angle_diff+a

    r = np.sin(new_a)
    a = np.radians(45)
    y = np.radians(central_angle)
    n = np.sqrt(r**2*(1/np.sin(a))**2 + 2*r*(1/np.tan(a))*1/(np.sin(a))*np.sin(y) + np.sin(y)**2 + (1/np.tan(a))**2*np.sin(y)**2)

    A = 1.5046
    B = 0.00420

    lamb = np.sqrt(abs(B/(A-n)))
    return lamb


    refraction_angle(central_angle,wave)
...
print(wave_location(-np.arange(-7,7)))
